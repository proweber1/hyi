...
class MinimalData {
     ...
     * @param dockerImageName имя образа в котором запускается код
    ...
    runDocker(emitter, codeDirectory, runner, containerName, dockerImageName, afterDown) {
        exec(`docker run -v ${codeDirectory}:/shared-code --rm --name ${containerName} ${dockerImageName} /runners/${runner}`, (err, stdout, stderr) => {
